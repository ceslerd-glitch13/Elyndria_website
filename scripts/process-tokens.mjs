...
async function buildManifest(tokensDirectory) {
  ...
  for (const filename of files) {
    ...
    if (metadata.format !== expectedFormat || metadata.width !== POLICY.width || metadata.height !== POLICY.height) {
      throw new Error(`${filename} is not a compliant ${POLICY.width} x ${POLICY.height} ${expectedFormat.toUpperCase()} asset`);
    }
  ...
  const formats = [...new Set(assets.map((asset) => asset.format))].sort();
  ...
  return {
    ...
    limits: {
      ...
      format: formats.length === 1 ? formats[0] : formats.length > 1 ? "mixed" : POLICY.format,
      formats
    },
    summary: {
